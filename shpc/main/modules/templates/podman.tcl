...
proc ModulesHelp { } {
    ...
    puts stderr "       podman run -it --rm --entrypoint {{ shell }}{% if envfile %} --env-file  {{ module_dir }}/{{ envfile }}{% endif %} {% if bindpaths %}-v {{ bindpaths }} {% endif %} -v . -w . <container> -c \"$@\""
...
set shellCmd "podman \${PODMAN_OPTS} run \${PODMAN_COMMAND_OPTS} --rm -it --entrypoint {{ shell }} {% if envfile %}--env-file {{ module_dir }}/{{ envfile }}{% endif %} {% if bindpaths %}-v {{ bindpaths }} {% endif %} -v . -w . ${containerPath}" 

# execCmd needs entrypoint to be the executor
set execCmd "podman \${PODMAN_OPTS} run -it \${PODMAN_COMMAND_OPTS} --rm {% if envfile %} --env-file {{ module_dir }}/{{ envfile }}{% endif %} {% if bindpaths %}-v {{ bindpaths }} -v . -w . {% endif %} "
set runCmd "podman \${PODMAN_OPTS} run -it \${PODMAN_COMMAND_OPTS} --rm {% if envfile %}--env-file  {{ module_dir }}/{{ envfile }}{% endif %} {% if bindpaths %}-v {{ bindpaths }} {% endif %} -v . -w . ${containerPath} $@"
